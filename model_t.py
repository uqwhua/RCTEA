...
class icl_loss(nn.Module):
    ...
    def __init__\
    (self, tau=0.05, ab_weight=0.5, n_view=2, intra_weight=1.0, \
...
class OverAll(nn.Module):
    def __init__(self, node_size, node_hidden, time_hidden,
        ...
        self.device = device
        self.time_adj, self.time_int_adj = \
            map(lambda x: x.to(device), [self.time_adj, self.time_int_adj])
